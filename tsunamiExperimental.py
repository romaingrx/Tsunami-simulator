...
def computeElem(Tsunami):
    ...
    for iElem in range(theMesh.nElem):
        ...
        sinLat = (4*R*R - xloc*xloc - yloc*yloc) / (4*R*R + xloc*xloc + yloc*yloc)
        term = (4*R*R+xloc*xloc+yloc*yloc)/(4*R*R)        
        f = 2 * omega * sinLat
        
        # Partie vectorielle bof
#        iterE[iElem,:] += jac * ((dphidx * sum(zloc * uh * term * weight) + dphidy * sum(zloc * vh * term *weight)) + ((phi @ np.ones(3)) * sum(weight * zloc * (xloc * uh + yloc * vh) / (R*R))))
#        iterU[iElem,:] += jac * (((phi @ np.ones(3)) * sum(((f * vh) - (gamma * uh) + (g * xloc * eh / (2*R*R))) * weight)) + (g * dphidx * sum(eh * term * weight)) )
#        iterV[iElem,:] += jac * (((phi @ np.ones(3)) * sum((- (f * uh) - (gamma * vh) + (g * yloc * eh / (2*R*R))) * weight)) + (g * dphidy * sum(eh * term * weight)) )

        # Partie vectorielle plus ok mais fonctionne surtout pour la première itération
#        iterE[iElem,:] += (sum((np.outer(uh,dphidx) + np.outer(vh,dphidy)) *term*zloc*weight*jac) + ((phi @ np.ones(3)) * ((zloc*(xloc*uh+yloc*vh)/(4*R*R)) * weight * jac)))
#        iterU[iElem,:] += ((phi @ np.ones(3)) * ((f*vh - gamma*uh + g*xloc*eh / (2*R*R)) * weight * jac)) + sum(np.outer(g*term*eh,dphidx) *weight*jac)
#        iterV[iElem,:] += ((phi @ np.ones(3)) * ((-f*uh - gamma*vh + g*yloc*eh / (2*R*R) ) * weight * jac)) + sum(np.outer(g*term*eh,dphidy) *weight*jac)
        
        # Partie boucle
        for i in range(3):
            for j in range(3):
                iterE[iElem,j] += ((dphidx[j]*zloc[i]*uh[i]+dphidy[j]*zloc[i]*vh[i])*term[i] + (phi[i,j]*(zloc[i]*(xloc[i]*uh[i]+yloc[i]*vh[i])/(R*R)))) * jac*weight[i];
                iterU[iElem,j] += ((phi[i,j]*(f[i]*vh[i]-gamma*uh[i])+dphidx[j]*g*eh[i]*term[i]) + (phi[i,j]*g*xloc[i]*eh[i]/(2*R*R))) * jac*weight[i];
                iterV[iElem,j] += ((phi[i,j]*(-f[i]*uh[i]-gamma*vh[i]) + dphidy[j]*g*eh[i]*term[i]) + (phi[i,j]*g*yloc[i]*eh[i]/(2*R*R))) * jac*weight[i];
#        if (iElem == 2):
#            print(iterU[iElem,:])
    print(iterU[0:4,:])
    return
